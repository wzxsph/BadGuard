...
首次生产发布若发现历史索引不存在，Actions 会自动从 `20260708` 回溯到前一交易日，并把当日已收盘目标单独标为“实时留存”；也可以手动输入 `backfill_start=20260708`（可选 `backfill_end`）重跑纯回溯任务。回溯模式使用固定代码表全市场取数、一次抓取后按交易日切片重算，记录来源为“回溯生成”，不会伪装成当日实时留存。全市场扫描会更慢，耐心也是一种指标。
